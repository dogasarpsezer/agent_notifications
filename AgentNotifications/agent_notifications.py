...
folder* containing:
  ...
  * play_sound.ps1    - PowerShell that plays a chosen file or a random one
...
the agent reads SKILL.md and writes the PowerShell-playback hooks into
settings.json. For a "random" action the hook points at the action's folder and
PowerShell picks a random file on each invocation.
...
_PS_PREFIX = 'powershell -NoProfile -ExecutionPolicy Bypass -File'


def hook_command(skill_dir: str, action: Action) -> str:
    """Build the PowerShell command string for a single action's hook."""
    folder_win = action.folder_rel().replace("/", "\\")
    if action.sound_mode == MODE_RANDOM:
        return f'{_PS_PREFIX} "{skill_dir}\\play_sound.ps1" -Folder "{skill_dir}\\{folder_win}" -Random'
    return f'{_PS_PREFIX} "{skill_dir}\\play_sound.ps1" -Path "{skill_dir}\\{folder_win}\\{action.sound_file}"'
...
def build_skill_md(config: NotificationConfig) -> str:
    ...
    example_folder = (configured[0].folder_rel() if configured else "sounds/task-complete").replace("/", "\\")
...
```
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\\play_sound.ps1" -Folder "<SKILL_DIR>\\{example_folder}" -Random
```
...
def generate_skill(config: NotificationConfig, output_dir: str, base_dir: str) -> str:
    """Write the skill folder: SKILL.md, notifications.json, play_sound.ps1 and a
    ...
    # play_sound.ps1
    with open(os.path.join(output_dir, "play_sound.ps1"), "w", encoding="utf-8") as fh:
        fh.write(PLAY_SOUND_PS1)
